...
# The tiles the interface shows, in order, including the ones a given document
# has none of: "SSN 0" says the type is implemented and simply absent.
CATEGORIES = (
    ("PERSON", "PERSON"),
    ("EMAIL", "EMAIL"),
    ("PHONE", "PHONE"),
...
class JobStore:
    ...
    def create(self, filename: str, upload) -> Job:
        self.prune()
    ...
    def _work(self, job: Job, source: Path) -> None:
        ...
        try:
            ...
            job.stats = _stats(
                stats.by_label, len(job.mapping), (time.perf_counter() - started) * 1000
            )
            job.status = "done"
...
def _stats(by_label: Counter, unique_mappings: int, elapsed_ms: float) -> dict:
    counts = {category: by_label.get(label, 0) for category, label in CATEGORIES}
    return {
